...
### Phase 2 — Curriculum layer (still deployable as a static site)

1. **Lesson framework** — markdown-authored lessons rendered into a
...
2. **Module plan** (roughly in teaching order):
...
3. **Per-module quizzes** with a shared question-bank format (JSON), local scoring.
