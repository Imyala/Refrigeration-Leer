...
test(">? renders a model answer the learner has to open", () => {
  const html = MD.render(">? **Four tests.**\n>? - continuity\n>? - insulation resistance");
  assert.match(html, /<details class="reveal">/);
  assert.match(html, /<summary>Show a model answer<\/summary>/);
  assert.match(html, /<li>continuity<\/li>/);
  assert.ok(!html.includes('class="callout"'), "a model answer is not a plain callout");
});

test("> and >! callouts still render as callouts, not reveals", () => {
  assert.match(MD.render("> a note"), /<div class="callout">/);
  assert.match(MD.render(">! a warning"), /<div class="callout warn">/);
  assert.ok(!MD.render("> a note").includes("<details"));
});
