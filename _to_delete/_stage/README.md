...
**54-module course** (437 lessons and 1,778 quiz questions with saved
...
The course is organised into four **streams**: a guided **core program**, two
technical refrigeration and air-conditioning streams, and an **electrical
principles** stream. See [The course](#the-course-learnhtml) below, and
...
- **`index.html`** — "Start here": the front door. The staged pathway through
  the core program, plus cards for the three specialist streams and the tools.
...
The three **specialist streams** then carry the syllabus to trade-course
depth:
...
A structured curriculum of **54 modules / 437 lessons / 1,778 quiz
questions**, written for learners working toward field competence. Modules
are grouped into four **streams**, declared in `js/streams.js`:
...
`docs/SOURCE_COVERAGE.md` lists all 54 modules with the source chapter each
...
(Refrigeration 1), `course201–213.js` (Refrigeration 2) and
`course301–314.js` (Electrical). Add a lesson by adding an object (content,
minutes, refs, quiz) to a module's `lessons` array; the navigation, progress
tracking and quizzes pick it up automatically.
...
- **Stream exams** (`#exam/<streamId>`) — each stream has its own exam drawing
  **two questions at random from every module of that stream**: 22 questions
  for the core program, 32 for Refrigeration 1, 26 for Refrigeration 2 and 28
  for Electrical principles.
- **Final exam** (`#exam`, sidebar or the last card on the course home) — the
  whole-program paper, **one question per module**, 54 in all, so it stays a
...
- **Curriculum mapping** — see `docs/CURRICULUM_MAPPING.md` for indicative
  mapping of all four streams to the UEE training package (Cert III RAC),
...
| `js/md.js` | Markdown-subset renderer for lesson content (incl. !SIM and !FIG directives) |
| `js/figures.js` | Inline SVG figure library used by `!FIG[id]` |
| `js/streams.js` | The four stream definitions; a module with no `stream` is `core` |
| `js/course1.js` … `js/course314.js` | Course content: 54 modules, 437 lessons, 1,778 questions (`course1–3` core, `course101–116` Refrigeration 1, `course201–213` Refrigeration 2, `course301–314` Electrical) |
