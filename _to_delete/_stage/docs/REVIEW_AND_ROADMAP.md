...
- **Phase 2 — done, and since expanded.** Structured course (`learn.html`):
  **54 modules / 437 lessons / 1,778 quiz questions**, grouped into four
  ...
  and Electrical principles (14 / 135). Per-lesson quizzes, explanations, saved
  progress and simulator deep links throughout; content authored in a markdown
  subset with validation tests. Full breakdown and source audit trail in
...
- **Phase 3 — static-feasible parts done.**
  - **SCORM 1.2 package** (`npm run build:scorm`): imports into Moodle/Canvas/
    ...
    Progress is encoded compactly (5 characters per lesson), so a fully
    complete 437-lesson course occupies 2,206 of SCORM 1.2's 4,096-character
    suspend_data budget — the earlier overflow concern is resolved, and the
    size is covered by a test against the whole syllabus.
  - **Exams**: a whole-program final exam (54 questions, one random question
    per module) plus one exam per stream (two per module of that stream), all
    at an 80% pass mark, and a printable **certificate of completion** with a
...
54 modules / 437 lessons / 1,778 questions across four streams, written from
...
2. **Module plan** — *done, and superseded by the four-stream syllabus.* The
...
3. **Per-module quizzes** with a shared question-bank format (JSON), local
   scoring — *done*; 1,778 questions, which also feed the stream and final exams.
...
4. **Consider the individual-learner channel** (apprentices, licence-exam prep)
   as a parallel revenue stream — same content, self-serve subscription.
