...
- **Ten realistic faults** with correct gauge signatures, and **Technician
  ...
  want.
- **Structured course** — **54 modules / 437 lessons / 1,778 quiz questions**
  (roughly 560,000 words), grouped into four streams:
...
- **Assessment** — an exam for each stream (two questions per module: 22, 32,
  26 and 28 questions) plus a whole-program 54-question final exam, all drawn
  at random from the 1,778-question bank, 80% pass, fresh paper every attempt,
...
1. **It is a whole syllabus, not a topic.** One platform now carries a full
   ...
   **second-year** course, and a full **electrical principles** course, on top
   of the guided core program — 437 lessons written from the standard
...
2. **It behaves like a real system.** Faults reshape pressures, superheat,
   subcooling, discharge temperature, COP and the P–h cycle coherently —
   students learn signatures, not slogans.
3. **Diagnosis is practised, not described.** The Technician Quiz gives a
   student fifty faulty systems in an afternoon; a workshop gives them two.
4. **Zero infrastructure.** Static files: no server, no accounts, no student
   data held by the vendor. IT approval is trivial.
5. **Fits how institutions already deliver** — SCORM into the existing LMS,
...
1. **Week 0 — setup (an hour):** import the SCORM zip into the LMS (or share
   ...
   units) and the two modules matching the term's delivery plan.
...
- Further expansion: more refrigerants (R32, CO2), electrical circuit
  ...
  first-year, second-year and electrical streams are complete.)
