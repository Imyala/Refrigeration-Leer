...
The package contains the whole platform: all 54 course modules (437 lessons)
across the four streams, the simulator, the Service Bay and the instructor
page. It is a single SCO of roughly **1.8 MB zipped** (about 5 MB unpacked) —
...
| SCORM field | Value |
|-------------|-------|
| `cmi.core.lesson_status` | `incomplete` → `completed` (all 437 lessons of the whole program passed) → `passed` (lessons complete **and** the whole-program final exam ≥ 80%) |
| `cmi.core.score.raw` (0–100) | The **final-exam best score in %** once the whole-program final exam has been attempted; before that, the % of the 437 lessons completed. Stream exams are recorded in the learner's progress but are not what the gradebook score reports |
...
437-lesson syllabus fits inside it. Progress is stored in a compact encoding
...
Measured against the real course: a learner who has completed **every one of
the 437 lessons** plus the whole-program final exam encodes to **2,206
characters**, and **2,274** with all four stream exams recorded as well —
roughly half the 4,096 budget either way. The same record in the previous
per-lesson text format was 21,082 characters. The size budget is asserted by
`tests/scorm.test.js` against the full syllabus, so it cannot silently regress
as content is added.
...
- **Exams:** the course has one exam per stream (two questions per module of
  that stream) and a whole-program final exam (one question per module, 54
  questions). All use an 80% pass mark. Only the **final** exam's score is
...
| Worried the record is too big for `suspend_data` | It isn't — progress is stored compactly and there is no per-lesson limit in practice: a fully complete 437-lesson course uses about 2,200 of the 4,096 characters SCORM 1.2 allows. See section 3. |
