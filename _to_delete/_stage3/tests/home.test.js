/* The home page groups the core-program modules into ordered stages. If a module id
...
const S = require(path.join(ROOT, "js", "streams.js"));
/* The pathway stages the CORE program. The specialist streams (Refrigeration 1
   and 2, Electrical) are reference-depth material reached from the course page,
   not stages of the guided path, so they are excluded here. */
const COURSE = fs
  .readdirSync(path.join(ROOT, "js"))
  .filter((f) => /^course\d+\.js$/.test(f))
  .sort((a, b) => a.localeCompare(b, "en", { numeric: true }))
  .flatMap((f) => require(path.join(ROOT, "js", f)))
  .filter((m) => S.streamIdOf(m) === "core");
