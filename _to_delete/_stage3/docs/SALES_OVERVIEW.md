...
- **Ten realistic faults** with correct gauge signatures, and **Technician
  ...
  want.
- **Service Bay** — the gauge-fitting job from the van in: choose a hose set
  (four are offered, one is right — the others fail on pressure rating,
  condition or missing low-loss ends), prove the gauges read zero, then caps,
  hoses, cracking the valves off the back seat, purging, reading and packing
  up as found. The **order of work** is graded against the eight taught steps,
  and any step done out of turn is marked and explained — because doing the
  same actions in the wrong order still gets you readings, and still puts air
  in the system or vents refrigerant.
- **System Builder** — students pipe a circuit themselves, dragging from a
  palette of thirty components into a closed loop in flow order. A rules
  engine decides whether that circuit would run and, when it would not, says
  why and where the component belongs instead: it knows the four runs of pipe
  (discharge, liquid, distribution, suction) and checks both that each
  accessory is in the right run and that it is on the right side of its
  neighbours. Findings are errors (it will not run), warnings (it runs, but a
  technician would have piped it differently) or good-practice tips, each with
  the reasoning attached. Five briefed scenarios — simplest working system,
  commercial cool room, pump-down circuit, reverse-cycle heat pump,
  low-temperature system with capacity protection — plus free build and an
  "explain this circuit" walk-through.
- **Fault Diagnosis Workshop** — the fault is hidden **and so are the
  numbers**. Students choose what to measure from thirteen points on the
  schematic, each naming the instrument and why the reading is worth taking;
  seven derived values (superheat, total superheat, subcooling, condenser TD,
  evaporator TD, air split, condenser air rise) appear only once the readings
  they depend on have been taken, and until then the page says what is missing
  and why. The evidence panel names the classic signatures — high superheat
  with low subcooling is undercharge, high superheat with high subcooling is a
  restriction — and scoring rates the **method** as well as the answer:
...
4. **Diagnosis is practised, not described.** The Technician Quiz gives a
   ...
   The Fault Diagnosis Workshop goes one step further and withholds the
   numbers as well as the fault, so the student practises the decision that
   actually separates technicians — *what do I measure next, and what would
   that change?* — and is marked on it.
5. **The procedural skills get repetitions too.** Piping a circuit and hooking
   up a gauge manifold are ordinarily bench-only exercises, rationed by plant
   and instructor time. The System Builder and the Service Bay give a student
   unlimited attempts at both, with the reasoning attached to every mistake,
   before they touch a machine — and both grade the thing bench marking is
   worst at capturing: whether the *order* and the *placement* were right, not
   just the finished result.
6. **Zero infrastructure.** Static files: no server, no accounts, no student
   data held by the vendor. IT approval is trivial.
7. **Fits how institutions already deliver** — SCORM into the existing LMS,
