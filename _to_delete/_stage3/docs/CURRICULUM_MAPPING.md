...
How the course's 64 modules relate to formal training frameworks. This
...
Refrigerant Handling Code of Practice, AS/NZS 3000/5149/4836, the ARAC
manuals Vols 1 & 2, and *Electrical Principles for the Electrical Trades*)
in an in-lesson references panel — see `docs/REFERENCES.md` for the master
bibliography, the alignment policy and the originality statement, and
...
|--------|--------:|--------:|----------:|------------|
| **Core program** | 11 | 39 | 123 | The guided pathway, simulator-linked throughout |
...
| 10 | Safety | 2 | — |
| 11 | The Refrigerant Handling Code of Practice | 10 | Reference library |
...
Five tools sit alongside the lessons. Each is referenced from the core-program
table above against the module it belongs with; all five are open at any time
and can be set as activities in their own right.

| Tool | What the learner does | Skill it drills |
|------|----------------------|-----------------|
| **Simulator** (`simulator.html`) | Drives a live vapour-compression cycle, applies faults, reads gauges and the P–h diagram | Cycle behaviour, instrument reading, off-design behaviour |
| **Technician Quiz** (simulator, quiz mode) | Names a hidden fault from the gauge picture | Signature recognition, scored repetition |
| **Service Bay** (`service.html`) | Chooses a hose set off the van, proves the gauges read zero, fits, cracks, purges, reads and packs up — graded on the order of work against the eight taught steps | Service-procedure discipline, emission control, gauge hook-up |
| **System Builder** (`build.html`) | Pipes a circuit from a palette of thirty components in flow order, to a written brief or free-build; every misplacement is explained by run (discharge / liquid / distribution / suction) and by order within the run | Reading and producing a piping schematic; where accessories belong and why |
| **Diagnosis Workshop** (`diagnose.html`) | Chooses which of thirteen measurement points to instrument, works superheat, subcooling and TDs from readings actually taken, then commits to a diagnosis — scored on method as well as answer | Systematic fault-finding with instruments; measurement technique |

**Where the hands-on three sit against a training package.** The Service Bay, the
System Builder and the Diagnosis Workshop are not knowledge-evidence
...
schematic, hooking up a manifold in the taught order, and working a fault
methodically with instruments are performance tasks, and the tools produce a
record of how the learner did them — which components went where and why the
placement was wrong, which step was taken out of turn, which readings were
taken before a diagnosis was committed to. They therefore map to the
**practical side** of a training package rather than to its knowledge
evidence, and are worth listing as such when planning delivery.

Two cautions, in the same spirit as the rest of this document. These are
simulations: they do not replace supervised workshop time, they do not
demonstrate psychomotor competence on real plant, and no RTO should treat
their output as practical assessment evidence on its own. And this mapping
remains **indicative** — a planning aid, not a compliance document. What the
tools do well is give a learner the repetitions and the reasoning *before*
scarce bench time, and give an instructor something concrete to talk about
when the learner reaches the bench.
...
practical/workshop assessment those units require. The Service Bay, System
Builder and Diagnosis Workshop are the nearest thing to it — see *The
interactive tools* above for what they do and do not stand in for.

Bare numbers below are **core-program** modules; `R1.x` / `R2.x` / `E.x` are
modules of the Refrigeration 1, Refrigeration 2 and Electrical principles
streams (see the tables above). The **capstone** stream (`C.x`) is
deliberately absent from the table below — see *Capstone stream* underneath
it.

| Skills area (indicative superseded unit) | Course modules |
|-------------------------------------------|----------------|
| Establish basic operating conditions of vapour-compression systems (UEENEEJ104A) | 1, 2, 5 + R1.1, R1.2–R1.6 (component behaviour), R2.7 + simulator & PT trainer |
| Establish basic operating conditions of air-conditioning systems (UEENEEJ103A) | 2, 6 + R2.3 (psychrometrics), R2.4 (system types), R1.7 |
| Prepare and connect refrigerant tubing and fittings (UEENEEJ102A) | 9 + **R1.8** (hand & power tools, tube preparation), **R1.9** (specialised tools), **R2.2** (piping, line sizing, oil return) + System Builder |
| Brazing, silver soldering and joining | **R1.10** (11 lessons: gas equipment, joint preparation, brazing and welding technique, safety) + 9 (nitrogen purge) |
| Recover, pressure test, evacuate, charge and leak test refrigerants (UEENEEJ108A) | 9 (all four lessons), 4, 11 + **R2.5** (service procedure), R1.9, R2.9 + Service Bay (hose selection, gauge hook-up, purging, order of work) |
| Fault-finding in vapour-compression systems | 5, 8 + **R2.7** (charts, tables, cycle analysis), **R2.8** (service diagnosis & repair), R1.15 + Technician Quiz, Diagnosis Workshop |
| Install, commission and maintain systems | **R2.9** (installation & maintenance), **R2.10** (commissioning & system balancing), R1.7 |
| Estimate heat load and select equipment | **R2.6** (load estimating & equipment selection), R2.3, R2.4 |
| Interpret and produce technical documentation and drawings | **R2.11** (technical communication), **R2.12** (drawing interpretation), R2.13 + System Builder (piping schematics) |
| Electrical fundamentals / motor starting for RAC | 7 + **R1.11–R1.16** (principles, components, wiring, motors, testing, instruments) + **E.2–E.7**, **E.9–E.12** |
| Develop and connect control circuits / motor control | **E.13** (develop and connect electrical control circuits), **R2.13** (controls and control drawings), R1.12 |
| Electrical measurement and test instruments | **R1.16**, **E.4**, E.14 + R1.15 |
| Trade calculations | **E.14**, E.3, R2.6 |
| WHS / safe work practices | 10 (+ safety callouts throughout) + **E.1** (working safely in the energy sector), R1.8 |
| Environmentally sustainable work practices | 4, 11 + **E.8** (sustainable practice in the energy sector) |

**Capstone stream.** The capstone stream does **not** map to any unit's
knowledge evidence, and it is not listed in the table above. It maps to
**assessment preparation**: it revises knowledge the learner has already met
in the streams above, in the form and at the depth of the written knowledge
assessment sat at the end of the qualification. Treat it as revision an RTO
schedules *after* delivery, not as coverage of a unit — the units are covered
by the core, Refrigeration 1 & 2 and Electrical principles streams, and the
capstone stream adds no new unit coverage of its own. Its practice questions
are original material written to the assessment's published scope; the course
does not hold, reproduce or claim to predict any real assessment paper.

**Electrical trades.** The Electrical principles stream (14 modules, 135
lessons) follows a standard electrical-principles syllabus and supports the
underpinning-knowledge components of electrotechnology core units — circuit
theory, electromagnetism, machines, transformers, protection and control
circuits. The same caveat applies: it is knowledge evidence only, and your
RTO must map it to the current unit codes.

**ARCtick (refrigerant handling licence)** — core modules 4, 9, 10 and
especially **11** (the Refrigerant Handling Code of Practice, Parts 1 & 2,
taught clause by clause over 10 lessons), plus R2.5 and R2.9, cover the
...
| Relate saturation pressure and temperature; use property data | Module 1, R2.7, PT chart & trainer, per-fluid saturation tables |
| Evaluate off-design and fault behaviour | Modules 5, 8, R2.8, fault simulation, Technician Quiz, Diagnosis Workshop |
| Refrigerant selection, environmental and safety constraints | Module 4, R2.1 |
