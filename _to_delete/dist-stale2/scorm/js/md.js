/* =========================================================================
   ...
   > callouts (>! for warning callouts, >? for a model answer the learner
   reveals after attempting the question themselves), **bold**, *italic*,
   [links](url),
  ...
  function render(md) {
    ...
    while (i < lines.length) {
      ...
      if (/^>/.test(line)) {
        ...
        /* >? is a model answer: hidden until the learner asks for it, so a
           written-answer question can be attempted before it is marked. */
        const reveal = /^>\?/.test(line);
        const buf = [];
        ...
        out.push(reveal
          ? `<details class="reveal"><summary>Show a model answer</summary>${body}</details>`
          : `<div class="callout${warn ? " warn" : ""}">${body}</div>`);
        continue;
      }
